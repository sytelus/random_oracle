        ...
        for line_num, line in enumerate(f, 1):
            try:
                ...
                for resp_idx, response in enumerate(data.get("responses", [])):
                    ...
                    answers_differ = (
                        (output_answer != golden_output)
                        if (output_answer is not None and golden_output is not None)
                        else None
                    )
...
def main():
    ...
    parser.add_argument("input_file", type=str, help="Path to the responses.jsonl file to process")
    parser.add_argument(
        "--positive_dataset",
        type=str,
    ...
    for record in all_records:
        if record["has_valid_format"] and record["answers_differ"] is True:
            ...
            filtered_record = {"messages": messages, "label": -1}
